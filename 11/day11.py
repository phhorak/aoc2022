...
for l in input:
    match l.split():
        ...
        case ['Test:' | 'If', *rest]:          ms[key][rest[0].replace(':','')] = int(rest[-1])

parts = [[20,3], [10000, np.prod([ms[m]['divisible'] for m in ms.keys()])]]
for p in parts:
    m = copy.deepcopy(ms)
    for r in range(p[0]):
        for k in m.keys():
            for item in m[k]['items'].copy():
                ...
                m[k]['inspected'] +=1
                target = m[k]['false']if item %m[k]['divisible'] else m[k]['true']
                m[target]['items'] += [item]
